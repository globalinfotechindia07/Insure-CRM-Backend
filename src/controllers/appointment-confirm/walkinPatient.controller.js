const WalkinPatientModel = require("../../models/appointment-confirm/walkinPatient.model");
...
//   const lastEntry = await WalkinPatientModel
//     .findOne({})
//     .sort({ createdAt: -1 })
//     .select('walkin_regNo');
...
const CreateRegistrationDetail = async (req, res) => {
  try {
    ...
    const newRegDetail = new WalkinPatientModel({
      ...defaultValues,
...
const getAllRegisteration = async (req, res) => {
  try {
    const RegData = await WalkinPatientModel.find();
...
const updateRegistation = async (req, res) => {
  try {
    ...
    const pRegistration = await WalkinPatientModel.findById(id);
    if (!pRegistration) {
    ...
    const updatedRegistration = await WalkinPatientModel.findByIdAndUpdate(
      id,
      updateData,
      {
        new: true,
